...
| **Multi-Format Documents** (compliance §6) | Canonical text hash | As of extension v1.5.0 the Chrome extension computes `content_hash_canonical` directly on-device for plain text, Markdown, CSV/TSV, JSON, XML, HTML, DOCX, XLSX, PPTX, and common PDFs. No desktop agent needed. For file types the extension can't parse (complex PDFs, archives, unrecognized formats), the self-hosted desktop agent at [self-hosted/aiauth_canonical.py](../self-hosted/aiauth_canonical.py) remains available as a fallback. | `SELECT content_hash_canonical, COUNT(DISTINCT hash) HAVING count > 1` — the canonical hash identifies the same logical document across format conversions. |
